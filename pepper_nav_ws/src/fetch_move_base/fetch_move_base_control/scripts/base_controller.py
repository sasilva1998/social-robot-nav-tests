...
Purpose: Alternative Fetch base controller 
"""

# ROS imports
import roslib; roslib.load_manifest('fetch_move_base_control')
import rospy
import tf
import tf2_ros
import math
...
from fetch_move_base_msgs.msg import Path2D
...
class Controller(object):
	...
	def __init__(self):
		...
		self.control_path_topic_ = rospy.get_param("~control_path_topic", "control_path_topic")
		self.control_output_topic_ = rospy.get_param("~control_output_topic", "control_output_topic")
		self.control_active_topic_ = rospy.get_param("~control_active_topic", "control_active_topic")
		self.xy_goal_tolerance_ = rospy.get_param("/fetch_move_base_planner/xy_goal_tolerance", 0.2)
		self.yaw_goal_tolerance_ = rospy.get_param("/fetch_move_base_planner/yaw_goal_tolerance", 0.2)
		
		#=======================================================================
		# Subscribers 
		#=======================================================================
		#Navigation data (feedback)
		self.odometry_sub_ = rospy.Subscriber(self.odometry_topic_, Odometry, self.odomCallback, queue_size = 1)
		self.control_path_sub_ = rospy.Subscriber(self.control_path_topic_, Path2D, self.receiveControlPathCallback, queue_size = 1)
		
		#=======================================================================
		# Publishers
		#=======================================================================
		self.control_output_pub_ = rospy.Publisher(self.control_output_topic_, Twist, queue_size = 1)
		self.control_active_pub_ =  rospy.Publisher(self.control_active_topic_, Bool, queue_size = 1)
		
		return
	
	def odomCallback(self, odometry_msg):
		"""
    	Callback to Fetch's odometry data
    	"""
	...
	def receiveControlPathCallback(self, path_2d_msg):
		...
		for i in range(len(path_2d_msg.waypoints)):
			...
			distance_to_wp = math.sqrt(math.pow(waypoint.x - self.current_position_[0], 2.0) + math.pow(waypoint.y - self.current_position_[1], 2.0))
			if distance_to_wp > self.xy_goal_tolerance_ or i == (len(path_2d_msg.waypoints) - 1):
			...
			if len(self.solution_path_wps_) > 0:
				...
				if distance_to_goal >= 0.3:
					...
					if abs(yaw_error) > 0.2:
						rospy.logdebug("%s: orienting towards the next waypoint", rospy.get_name())
						control_input.angular.z = yaw_error * self.max_turn_rate_
					else:
						rospy.logdebug("%s: moving towards the next waypoint", rospy.get_name())
						control_input.angular.z = yaw_error * self.max_turn_rate_
					...
					if len(self.solution_path_wps_) > 1:
						...
						if distance_to_goal >= self.xy_goal_tolerance_ and controller_state != 2:
							controller_state = 1
							self.desired_orientation_ = wrapAngle(math.atan2(inc_y, inc_x))
							yaw_error = wrapAngle(self.desired_orientation_ - self.current_orientation_)
		
							if abs(yaw_error) > 0.2 and abs(yaw_error) < 1.57:
								rospy.logdebug("%s: final approach: orienting towards a waypoint (fordward)", rospy.get_name())
								if yaw_error > 0.0:
								...
								rospy.logdebug("%s: final approach: orienting towards a waypoint (backward)", rospy.get_name())
								if yaw_error > 0.0:
									control_input.angular.z = -self.min_turn_rate_
								else:
									control_input.angular.z = self.min_turn_rate_
								...
								rospy.logdebug("%s: final approach: moving towards a waypoint (fordward)", rospy.get_name())
								
								control_input.linear.x = 0.05
							else:
								rospy.logdebug("%s: final approach: moving towards a waypoint (backward)", rospy.get_name())
							...
							controller_state = 2
							rospy.logdebug("%s: final approach, final orientation", rospy.get_name())
							yaw_error = wrapAngle(self.solution_path_wps_[0][2] - self.current_orientation_) 
