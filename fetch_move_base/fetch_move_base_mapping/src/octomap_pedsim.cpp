...
struct SocialAgentsExtended
{
  std::string topic;
  std::string frame;
  tf::StampedTransform tf_robot_odom;
  ros::Subscriber sub;
};
...
public:
  ...
  void agentsDataCallback(const pedsim_msgs::AgentStatesConstPtr &agent_states_msg);
...
private:
  ...
  // Social Agents
  std::vector<std::string> social_agents_topic, social_agents_frames_;
  std::vector<SocialAgentsExtended *> social_agents_info_;
      ...
      global_map_available_(false)
{
  ...
  if (offline_octomap_path_.size() == 0)
  {
    ...
    for (std::vector<SocialAgentsExtended *>::iterator social_agents_it =
             social_agents_info_.begin();
         social_agents_it != social_agents_info_.end(); social_agents_it++)
    {
